...
import { ArrowLeft, Send, MessageSquare, Search, Smile, ThumbsUp, Crown, Pencil, Trash2, Flag } from "lucide-react";
...
interface Conversation {
  id: string;
  other: { id: string; name: string; avatar: string; vip_tier?: string | null };
  last_message: string;
  last_at: string;
  unread: number;
}
...
function ChatPane({ conv, meId, onBack, isOnline }: { conv: Conversation; meId: string; onBack: () => void; isOnline: boolean }) {
  ...
  const editMsg = async (m: Msg) => {
    ...
    try { await api(`/api/conversations/${convId}/messages/${m.id}`, { method: "PATCH", json: { body: nb.trim() } }); qc.invalidateQueries({ queryKey: ["messages", convId] }); } catch { /* */ }
  };
  const reportMsg = async (m: Msg) => {
    const reason = prompt("Report this message — why?"); if (reason === null) return;
    try { await api("/api/report", { method: "POST", json: { target_type: "message", target_id: m.id, reason } }); alert("Thanks — reported."); } catch { /* */ }
  };
  const delMsg = async (m: Msg) => {
    if (!confirm("Delete this message?")) return;
    try { await api(`/api/conversations/${convId}/messages/${m.id}`, { method: "DELETE" }); qc.invalidateQueries({ queryKey: ["messages", convId] }); qc.invalidateQueries({ queryKey: ["conversations"] }); } catch { /* */ }
  };
    ...
    <div className="flex h-full w-full flex-col bg-deep">
      <div className="flex h-16 shrink-0 items-center gap-3 border-b border-white/5 bg-white/[0.03] px-4">
        ...
        <Link href={`/channel/${conv.other.id}`} className="flex items-center gap-3 transition-opacity hover:opacity-80">
          ...
          <div>
            <p className="flex items-center gap-1 font-semibold leading-tight text-white">{conv.other.name}{conv.other.vip_tier && <Crown size={13} className="text-fuchsia" />}</p>
      ...
      <div className="hide-scrollbar min-h-0 flex-1 overflow-y-auto bg-deep px-4 py-4">
        <div className="flex flex-col gap-1.5">
          {(msgs ?? []).map((m, idx) => {
            ...
            return (
              <div key={m.id} className={`group flex w-full items-center gap-2 ${mine ? "justify-end" : "justify-start"} ${isSequence ? "mt-0" : "mt-2"}`}>
                {mine && !isLike && (
                  <div className="hidden items-center gap-1 group-hover:flex">
                    <button onClick={() => editMsg(m)} className="rounded-full p-1 text-white/40 hover:bg-white/10 hover:text-white"><Pencil size={13} /></button>
                    <button onClick={() => delMsg(m)} className="rounded-full p-1 text-white/40 hover:bg-white/10 hover:text-rose"><Trash2 size={13} /></button>
                  </div>
                )}
                {!mine && !isLike && (
                  <button onClick={() => reportMsg(m)} className="hidden rounded-full p-1 text-white/40 hover:bg-white/10 hover:text-white group-hover:block" title="Report"><Flag size={13} /></button>
                )}
                {!mine && (showAvatar
                  ? // eslint-disable-next-line @next/next/no-img-element
                ...
                <button key={c.id} onClick={() => setActive(c.id)}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-0.5 flex items-baseline justify-between">
                      <p className={`flex items-center gap-1 truncate pr-2 text-[15px] ${unread ? "font-bold text-white" : "font-semibold text-white/90"}`}>{c.other.name}{c.other.vip_tier && <Crown size={12} className="shrink-0 text-fuchsia" />}</p>
                    </div>
