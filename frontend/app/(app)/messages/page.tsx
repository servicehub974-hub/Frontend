            ...
            return (
              <div key={m.id} className={`flex items-end gap-2 ${mine ? "justify-end" : "justify-start"}`}>
                ...
                {isLike ? (
                  ...
                  <div className={`max-w-[75%] whitespace-pre-wrap break-words rounded-2xl px-3.5 py-2 text-sm ${mine ? "bg-gradient-to-br from-cyan-600 to-violet-600 text-white" : "bg-white/10 text-white/90"}`}>
                    {m.body}
                  </div>
                )}
              </div>
...
function MessagesInner() {
  ...
  return (
    <div className="flex h-[calc(100dvh-4rem)] w-full overflow-hidden sm:h-[calc(100dvh-5rem)]">
      {/* list */}
      <div className={`flex w-full flex-col border-r border-white/5 sm:w-[360px] sm:shrink-0 ${active ? "hidden sm:flex" : "flex"}`}>
      ...
      {/* chat */}
      <div className={`min-w-0 flex-1 ${active ? "flex" : "hidden sm:flex sm:items-center sm:justify-center"}`}>
        {activeConv ? (
