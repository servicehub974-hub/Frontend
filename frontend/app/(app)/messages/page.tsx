...
function ChatPane({ conv, meId, onBack }: { conv: Conversation; meId: string; onBack: () => void }) {
  ...
  const { data: msgs } = useQuery({
    ...
    refetchInterval: 15000, // fallback; realtime handles instant
  });
...
function MessagesInner() {
  ...
  return (
    <div className="flex h-[calc(100dvh-4rem)] w-full overflow-hidden sm:h-[calc(100dvh-5rem)]">
      {/* list */}
      <div className={`flex w-full flex-col border-r border-white/5 sm:w-[340px] sm:shrink-0 ${active ? "hidden sm:flex" : "flex"}`}>
