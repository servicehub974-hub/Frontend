...
interface Page { items: Post[]; next_cursor: string | null; }

export default function FeedPage() {
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useInfiniteQuery({
    queryKey: ["feed"],
    queryFn: ({ pageParam }) => apiGet<Page>(`/api/posts${pageParam ? `?cursor=${encodeURIComponent(pageParam)}` : ""}`),
    initialPageParam: "",
    ...
    <div className="mx-auto max-w-2xl px-4 py-6 sm:px-6">
      ...
      <div className="mt-4 flex flex-col gap-4">
        {isLoading ? (
          ...
          <p className="py-10 text-center text-sm text-white/40">No posts yet. Be the first to share something!</p>
        ) : (
