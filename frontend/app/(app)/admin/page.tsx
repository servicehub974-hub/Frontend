...
import { Stats, UsersAdmin, Reports, SiteBranding, Audit, HiddenContent } from "@/components/admin/admin-moderation";
...
const TABS = ["Overview", "Users", "Reports", "Hidden", "Content", "Orders", "Monetization", "Features", "Site", "Feedback", "Audit"] as const;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${tab === t ? "bg-white text-black" : "bg-white/5 text-white/60 hover:bg-white/10"}`}>{t}</button>
        ))}
      </div>
      ...
      {tab === "Orders" && (
        <section>
          ...
          {!orders.data || orders.data.length === 0 ? <p className="text-sm text-white/40">No pending orders.</p> : (
            <div className="flex flex-col gap-2">
              {orders.data.map((o) => (
                <div key={o.id} className="flex flex-col gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-3 sm:flex-row sm:items-center">
