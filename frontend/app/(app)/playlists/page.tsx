...
export default function PlaylistsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 md:px-8">
      <PageHeader icon={ListVideo} title="Playlists" action={<Button size="sm" onClick={() => setCreating((v) => !v)}><Plus size={16} /> New</Button>} />
      ...
      {!data || data.length === 0 ? (
        <EmptyState icon={ListVideo} title="No playlists yet" subtitle="Create a playlist to organize videos into collections." action={<Button size="sm" onClick={() => setCreating(true)}><Plus size={16} /> New playlist</Button>} />
      ) : (
