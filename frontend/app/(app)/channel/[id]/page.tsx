...
import { CheckCircle, X, Share2, Flag, Users, Video, Eye, Calendar, Globe, Settings, MessageSquare, Crown } from "lucide-react";
...
interface PublicProfile {
  id: string; username: string | null; display_name: string | null; vip_tier?: string | null;
  ...
  joined: string | null; is_following: boolean; is_blocked?: boolean;
}

type Tab = "videos" | "reels" | "posts" | "playlists" | "about";
...
export default function ChannelPage({ params }: { params: { id: string } }) {
  ...
  const { data: content } = useQuery({
    ...
    enabled: tab !== "about" && tab !== "playlists" && tab !== "posts" && tab !== "reels",
  });
  ...
  const { data: posts } = useQuery({
    ...
    queryFn: () => apiGet<Post[]>(`/api/posts/user/${id}`),
    enabled: tab === "posts",
  });
  const { data: reels } = useQuery({
    queryKey: ["channel-reels", id],
    queryFn: () => apiGet<ContentItem[]>(`/api/users/${id}/content?kind=short`),
    enabled: tab === "reels",
  });
  ...
  useEffect(() => { if (p) { setFollowing(p.is_following); setFollowers(p.follower_count); setBlocked(!!p.is_blocked); } }, [p]);
  ...
  const toggleBlock = async () => {
    if (!user) return;
    const was = blocked; setBlocked(!was);
    try { const r = await api<{ blocked: boolean }>(`/api/users/${id}/block`, { method: "POST" }); setBlocked(r.blocked); } catch { setBlocked(was); }
  };
  const reportUser = async () => {
    if (!user) return;
    const reason = prompt("Report this user — why?"); if (reason === null) return;
    try { await api("/api/report", { method: "POST", json: { target_type: "user", target_id: id, reason } }); alert("Thanks — reported to moderators."); } catch { /* */ }
  };
  const shareChannel = async () => {
    const url = window.location.href;
    const nav = navigator as Navigator & { share?: (d: { title?: string; url?: string }) => Promise<void> };
    if (nav.share) { try { await nav.share({ title: p?.display_name || "Channel", url }); return; } catch { /* cancelled */ } }
  ...
  const tabs: Tab[] = ["videos", "reels", "posts", "playlists", "about"];
      ...
      <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-start">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            {p.vip_tier && <span className="flex items-center gap-1 rounded-full bg-gradient-to-r from-fuchsia-600/30 to-violet-600/30 px-2 py-0.5 text-[11px] font-bold uppercase text-fuchsia"><Crown size={12} /> {p.vip_tier}</span>}
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {isMe ? (
              <>
                ...
                <Link href="/creator/dashboard" className="flex items-center gap-2 rounded-full bg-white text-black px-5 py-2 text-sm font-semibold hover:bg-white/90">
                  Manage content
                </Link>
              </>
            ) : (
            ...
            <button onClick={shareChannel} className="rounded-full bg-white/10 px-5 py-2 text-sm font-semibold text-white hover:bg-white/15">{copied ? "Copied!" : "Share"}</button>
            {user && user.id !== id && (
              <>
                <button onClick={toggleBlock} className={`rounded-full px-5 py-2 text-sm font-semibold ${blocked ? "bg-rose/20 text-rose" : "bg-white/10 text-white hover:bg-white/15"}`}>{blocked ? "Blocked" : "Block"}</button>
                <button onClick={reportUser} title="Report" className="rounded-full bg-white/10 p-2 text-white/60 hover:bg-white/15 hover:text-white"><Flag size={16} /></button>
              </>
            )}
          </div>
        </div>
      ...
      <div className="mt-6 min-h-[45vh]">
        {tab === "reels" ? (
          !reels || reels.length === 0 ? (
            <p className="text-sm text-white/40">No reels yet.</p>
          ) : (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
              {reels.map((r) => (
                <Link key={r.id} href={`/reels?v=${r.id}`} className="group relative aspect-[9/16] overflow-hidden rounded-xl bg-elevated">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={r.thumbnail} alt={r.title} className="h-full w-full object-cover transition-transform group-hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                  <p className="absolute bottom-2 left-2 right-2 line-clamp-2 text-xs font-medium text-white">{r.title}</p>
                </Link>
              ))}
            </div>
          )
          ...
          <div className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {content.map((item, i) => <ContentCard key={item.id} item={item} index={i} owner={isMe} />)}
          </div>
        )}
      </div>
