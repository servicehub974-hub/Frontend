...
import { Heart, MessageCircle, Share2, CheckCircle, Lock, Gem, Play, Volume2, VolumeX } from "lucide-react";
...
interface Reel {
  ...
  creator: { name: string; avatar: string; verified: boolean }; creator_id: string;
    ...
    <div ref={wrapRef} className="relative flex h-full w-full snap-start items-center justify-center bg-black">
      <div className="relative h-full w-full max-w-[460px] overflow-hidden bg-black">
        {r.is_locked ? (
          ...
          <video ref={vidRef} src={r.media_url} className="h-full w-full object-contain" loop muted={muted} playsInline onClick={() => { const v = vidRef.current; if (v) v.paused ? v.play() : v.pause(); }} />
        ) : (
        ...
        <div className="absolute bottom-24 right-3 z-10 flex flex-col items-center gap-5 text-white">
          ...
          <Link href={`/content/${r.id}`} className="flex flex-col items-center"><MessageCircle size={28} /><span className="text-xs">{r.comment_count}</span></Link>
        ...
        <div className="absolute bottom-6 left-4 right-16 z-10 text-white">
          <div className="mb-2 flex items-center gap-2">
            <Link href={`/channel/${r.creator_id}`} className="flex items-center gap-2">
              ...
              <span className="flex items-center gap-1 text-sm font-semibold">{r.creator.name} {r.creator.verified && <CheckCircle size={12} />}</span>
            </Link>
