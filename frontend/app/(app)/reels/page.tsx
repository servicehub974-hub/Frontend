...
import { Heart, MessageCircle, Share2, CheckCircle, Lock, Gem, Play, Volume2, VolumeX, Crown, Bookmark } from "lucide-react";
...
interface Reel {
  ...
  like_count: number; comment_count: number; is_liked: boolean; is_following: boolean; is_saved?: boolean;
}
...
function ReelItem({ r, muted, onToggleMute }: { r: Reel; muted: boolean; onToggleMute: () => void }) {
  ...
  const save = async () => {
    ...
    try { const res = await api<{ saved: boolean }>(`/api/content/${r.id}/save`, { method: "POST" }); setSaved(res.saved); } catch { setSaved(was); }
  };
    ...
    <div ref={wrapRef} className="relative flex h-full w-full snap-start items-center justify-center bg-black">
      <div className="relative h-full w-full max-w-[460px] overflow-hidden bg-black">
        ...
        <div className="absolute bottom-24 right-3 z-10 flex flex-col items-center gap-5 text-white">
          ...
          <button onClick={save} className="flex flex-col items-center"><Bookmark size={26} className={saved ? "fill-white text-white" : ""} /><span className="text-xs">{saved ? "Saved" : "Save"}</span></button>
        </div>
...
function ReelsInner() {
  ...
  const v = sp.get("v");
  const { data: pinned, isLoading: pinnedLoading, isError: pinnedError } = useQuery({
  ...
  const reels: Reel[] = pinned
    ...
    : feedReels;
  const waitingPinned = !!v && pinnedLoading && !pinnedError;

  return (
    <div className="fixed bottom-16 left-0 right-0 top-16 z-20 overflow-y-scroll bg-black snap-y snap-mandatory hide-scrollbar sm:bottom-0 sm:top-20 sm:left-[72px] lg:left-64" style={{ overflowAnchor: "none" }}>
      {isLoading || waitingPinned ? (
