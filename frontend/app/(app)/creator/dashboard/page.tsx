...
interface PostItem { id: string; title: string | null; body: string | null; images: string[]; like_count: number; comment_count: number; repost_count: number; time_ago: string; }

const FILTERS = ["All", "Videos", "Reels", "Posts", "Premium", "Free", "Published", "Private"] as const;
...
export default function CreatorDashboard() {
  ...
  const { data: posts } = useQuery({ queryKey: ["manage-posts", user?.id], queryFn: () => apiGet<PostItem[]>(`/api/posts/user/${user!.id}`), enabled: !!user && filter === "Posts" });
  const delPost = async (id: string) => { if (!confirm("Delete this post?")) return; try { await api(`/api/posts/${id}`, { method: "DELETE" }); qc.invalidateQueries({ queryKey: ["manage-posts", user?.id] }); } catch { /* */ } };
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      {filter === "Posts" ? (
        !posts || posts.length === 0 ? (
          <p className="py-10 text-center text-sm text-white/40">No posts yet.</p>
        ) : (
          <div className="flex flex-col gap-3">
            {posts.map((p) => (
              <div key={p.id} className="glass-panel flex items-center gap-3 rounded-2xl p-3">
                {p.images && p.images[0] ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={p.images[0]} alt="" className="h-14 w-14 shrink-0 rounded-lg object-cover" />
                ) : (
                  <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-elevated text-white/30">Aa</div>
                )}
                <div className="min-w-0 flex-1">
                  <p className="line-clamp-1 text-sm font-semibold text-white">{p.title || p.body || "Post"}</p>
                  <div className="mt-1 flex items-center gap-3 text-xs text-white/50">
                    <span className="flex items-center gap-1"><Heart size={13} /> {fmtCount(p.like_count)}</span>
                    <span className="flex items-center gap-1"><MessageCircle size={13} /> {fmtCount(p.comment_count)}</span>
                    <span className="flex items-center gap-1"><Repeat2 size={13} /> {fmtCount(p.repost_count)}</span>
                    <span>{p.time_ago}</span>
                  </div>
                </div>
                <Link href={`/post/${p.id}`} className="rounded-full bg-white/10 px-3 py-1.5 text-xs font-medium text-white hover:bg-white/15">Open</Link>
                <button onClick={() => delPost(p.id)} className="rounded-full p-2 text-white/40 hover:bg-white/10 hover:text-rose"><Trash2 size={15} /></button>
              </div>
            ))}
          </div>
        )
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  {it.content_type === "video" && (
                    <button onClick={() => patch(it.id, { is_short: !it.is_short })} className={`rounded-md px-2 py-1 text-xs ${it.is_short ? "bg-cyan/20 text-cyan" : "bg-white/5 text-white/50"}`}>{it.is_short ? "Reel (9:16)" : "Video (16:9)"}</button>
                  )}
