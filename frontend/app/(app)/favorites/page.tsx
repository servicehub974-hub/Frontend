...
export default function FavoritesPage() {
  ...
  const { data: posts } = useQuery({ queryKey: ["saved-posts"], queryFn: () => apiGet<Post[]>("/api/posts/saved"), enabled: !!user });
  const { data, isLoading } = useQuery({
    queryKey: ["saved"],
    queryFn: () => apiGet<ContentItem[]>("/api/content/saved"),
    enabled: !!user,
  });

  if (loading) return <div className="px-6 py-16 text-white/50">Loading…</div>;
  if (!user) return <div className="px-6 py-16 text-center text-white/60">Log in to see your favorites.</div>;

  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 md:px-8">
      <PageHeader icon={Bookmark} title="Favorites" tint="text-gold" />
      {isLoading ? (
        <div className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="aspect-video" />)}
        </div>
      ) : !data || data.length === 0 ? (
        <EmptyState icon={Bookmark} title="No favorites yet" subtitle="Tap Save on any video to add it here." />
      ) : (
        <div className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
