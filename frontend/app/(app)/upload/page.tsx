...
import { uploadFile, readVideoMeta, captureVideoFrame } from "@/lib/storage";
...
export default function UploadPage() {
  ...
  const handleFile = async (file: File) => {
    ...
    setIsShort(meta.isVertical);
    setProgress(0);
    try {
      const url = await uploadFile(file, setProgress);
      setMediaUrl(url);
      setProgress(100);
      // auto-thumbnail: if the user hasn't picked one, grab a frame from the video
      if (!thumbUrl) {
        try {
          const blob = await captureVideoFrame(file, 1);
          if (blob) {
            const auto = new File([blob], "frame.jpg", { type: "image/jpeg" });
            const turl = await uploadFile(auto);
            setThumbUrl((prev) => prev || turl);
          }
        } catch { /* ignore auto-thumb failures */ }
      }
    } catch (e) {
    ...
    try {
      await api("/api/content", {
        ...
        json: {
          ...
          is_short: mediaType === "video" ? isShort : false,
    ...
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      ...
      {!uploaded ? (
        ...
        <div className="glass-panel flex flex-col gap-4 rounded-xl p-5 sm:p-6">
          ...
          {mediaType === "video" && (
            <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-3">
              <input type="checkbox" checked={isShort} onChange={(e) => setIsShort(e.target.checked)} className="h-4 w-4 accent-cyan-500" id="reel" />
              <label htmlFor="reel" className="flex-1 text-sm text-white/80">This is a Reel (9:16 vertical — plays in the Reels player)</label>
            </div>
          )}
          <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-3">
