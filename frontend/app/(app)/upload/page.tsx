...
import { uploadFile, readVideoDuration } from "@/lib/storage";
...
export default function UploadPage() {
  ...
  const handleFile = async (file: File) => {
    ...
    if (!isVideo && !isImage) return setError("Please choose a video or image file.");
    setMediaType(isVideo ? "video" : "photo");
    setFileName(file.name);
    if (!form.title) set("title")(file.name.replace(/\.[^.]+$/, ""));
    if (isVideo) setDurationSec(await readVideoDuration(file));
    setProgress(0);
    try {
    ...
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="flex items-center gap-3 text-2xl font-bold tracking-tight text-white">
          <UploadCloud size={24} className="text-cyan" /> Upload
        </h1>
        ...
        useLink ? (
          ...
          >
            ...
            {progress !== null ? (
              ...
              <>
                <p className="mb-1 text-white/80">Drag & drop a video or photo</p>
                <p className="mb-4 text-xs text-white/40">or</p>
                <Button onClick={() => fileInput.current?.click()}>Select file</Button>
              </>
            )}
            <input ref={fileInput} type="file" accept="video/*,image/*" className="hidden"
