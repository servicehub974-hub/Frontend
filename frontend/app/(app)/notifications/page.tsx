...
import { Bell, UserPlus, MessageSquare, ThumbsUp, CornerDownRight, Sparkles, Heart, Repeat2, Crown, Gem, Shield } from "lucide-react";
import { useAuth } from "@/providers/auth-provider";
import { api, apiGet } from "@/lib/api";

interface Notif { id: string; type: string; actor_name: string | null; actor_avatar: string | null; message: string; link: string | null; image: string | null; is_read: boolean; time_ago: string; }

function icon(type: string) {
  if (type === "follow") return <UserPlus size={18} className="text-cyan" />;
  if (type === "comment" || type === "post_comment") return <MessageSquare size={18} className="text-cyan" />;
  ...
  if (type === "post_like") return <Heart size={18} className="text-rose" />;
  if (type === "repost") return <Repeat2 size={18} className="text-cyan" />;
  if (type === "order") return <Crown size={18} className="text-gold" />;
  if (type === "purchase") return <Gem size={18} className="text-gold" />;
  if (type === "vip_expired") return <Crown size={18} className="text-white/40" />;
  if (type === "moderation") return <Shield size={18} className="text-rose" />;
          ...
          {data.map((n) => (
            <button key={n.id} onClick={() => open(n)}
              ...
              {n.actor_avatar ? (
                ...
                <img src={n.actor_avatar} alt="" className="mt-0.5 h-9 w-9 shrink-0 rounded-full object-cover" />
              ) : (
