...
export default function LibraryPage() {
  ...
  if (!user)
    return <div className="px-6 py-16 text-center text-white/60">Log in to see your saved content.</div>;

  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 md:px-8">
      <div className="mb-6 flex items-center gap-3">
        <Layers size={24} className="text-cyan" />
        <h1 className="text-2xl font-bold tracking-tight text-white">My Vault</h1>
      </div>
      {isLoading ? (
        ...
        <p className="text-sm text-white/40">Nothing saved yet. Tap “Save” on any video.</p>
      ) : (
        <div className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
