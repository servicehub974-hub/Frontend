...
function RelatedList({ currentId }: { currentId: string }) {
  ...
  if (items.length === 0)
    return <p className="text-sm text-white/40">No related content yet.</p>;
      ...
      {items.map((it) => (
        <Link key={it.id} href={`/content/${it.id}`} className="group flex gap-2">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="line-clamp-2 text-sm font-medium leading-snug text-white group-hover:text-white">
              {it.title}
            </h3>
        ...
        <div className="px-4 sm:px-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`ml-2 rounded-full px-4 py-2 text-sm font-semibold transition-colors ${
                  subscribed ? "bg-white/10 text-white" : "bg-white text-black hover:bg-white/90"
                }`}
              >
            ...
            <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar">
              <div className="flex shrink-0 items-center rounded-full bg-white/10">
                <button onClick={() => setLiked((v) => !v)}
                  className="flex items-center gap-2 rounded-l-full px-4 py-2 text-sm font-medium text-white hover:bg-white/15">
                  <ThumbsUp size={16} className={liked ? "fill-white" : ""} /> {liked ? "1" : "0"}
                </button>
                <div className="h-5 w-px bg-white/15" />
                <button className="rounded-r-full px-4 py-2 text-white hover:bg-white/15">
                  <ThumbsDown size={16} />
                </button>
              </div>
          ...
          <div className="mt-4 rounded-xl bg-white/5 p-3 text-sm">
            <div className="font-medium text-white/80">
              ...
              {d.category && (
                <Link href={`/?category=${d.category.slug}`} className="ml-2 text-cyan hover:underline">
                  #{d.category.name}
                </Link>
              )}
            </div>
            ...
            {d.tags.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {d.tags.map((t) => (
                  <span key={t} className="rounded-full bg-white/5 px-2.5 py-1 text-xs text-white/50">#{t}</span>
                ))}
              </div>
            )}
          </div>
