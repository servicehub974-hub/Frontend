...
function Media({ d, onNext }: { d: Detail; onNext?: () => void }) {
  ...
  return <VideoPlayer src={d.media_url} poster={d.thumbnail_url ?? undefined} onNext={onNext} />;
}
...
export default function ContentDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (d) {
      ...
      setFollowing(d.is_following); setFollowers(d.follower_count);
    }
  }, [d]);
  ...
  const nextId = relatedData?.items.find((i) => i.id !== id)?.id;
  const goNext = nextId ? () => router.push(`/content/${nextId}`) : undefined;
