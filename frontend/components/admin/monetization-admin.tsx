...
export function MonetizationAdmin() {
  ...
  const ok = (m = "Saved ✓") => { setFlash(m); setTimeout(() => setFlash(""), 1600); };
  ...
  const refetchTiers = () => { qc.invalidateQueries({ queryKey: ["admin-vip-tiers"] }); qc.invalidateQueries({ queryKey: ["vip-tiers"] }); ok(); };
  const refetchPkgs = () => { qc.invalidateQueries({ queryKey: ["admin-gem-packages"] }); qc.invalidateQueries({ queryKey: ["gem-packages"] }); ok(); };
  ...
  return (
    <>
      {flash && <div className="fixed bottom-6 left-1/2 z-[300] -translate-x-1/2 rounded-full bg-green-600 px-5 py-2 text-sm font-semibold text-white shadow-xl">{flash}</div>}
