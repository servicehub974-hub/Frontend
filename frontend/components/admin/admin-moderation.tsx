...
import { BarChart3, Users, Flag, ScrollText, Settings2, Ban, ShieldCheck, Check, X, ExternalLink, RotateCcw, EyeOff } from "lucide-react";
...
interface Rep { id: string; target_type: string; target_id: string; reason: string | null; preview: { title?: string; thumbnail?: string } | null; link?: string | null; created_at: string | null; }
    ...
    <section className="mt-8">
      ...
      <div className="flex flex-col gap-2">
        {list.slice(0, 50).map((u) => (
          <div key={u.id} className="flex flex-col gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-2.5 text-sm sm:flex-row sm:items-center">
  ...
  return (
    <section className="mt-8">
      ...
      {!data || data.length === 0 ? <p className="text-sm text-white/40">No open reports.</p> : (
        <div className="flex flex-col gap-2">
          {data.map((r) => (
            <div key={r.id} className="flex flex-col gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-2.5 sm:flex-row sm:items-center">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {r.link && <a href={r.link} target="_blank" rel="noreferrer" className="flex items-center gap-1 rounded-full bg-white/10 px-3 py-1.5 text-xs text-white hover:bg-white/15"><ExternalLink size={13} /> View</a>}
    ...
    <section className="mt-8">
      ...
      {!data || data.length === 0 ? <p className="text-sm text-white/40">No activity yet.</p> : (
        <div className="flex flex-col divide-y divide-white/5 text-sm">
          {data.map((a, i) => (
            <div key={i} className="flex flex-col gap-1 py-2 text-white/70 sm:flex-row sm:items-center sm:gap-2">
...
export function HiddenContent() {
  const qc = useQueryClient();
  const { data } = useQuery({ queryKey: ["admin-hidden"], queryFn: () => apiGet<{ id: string; title: string; thumbnail: string | null; is_short: boolean }[]>("/api/admin/hidden-content") });
  const restore = async (id: string) => { try { await api(`/api/admin/content/${id}/restore`, { method: "POST" }); qc.invalidateQueries({ queryKey: ["admin-hidden"] }); } catch { /* */ } };
  return (
    <section className="mt-8">
      <h2 className="mb-3 flex items-center gap-2 text-lg font-semibold text-white"><EyeOff size={18} className="text-white/50" /> Hidden content</h2>
      {!data || data.length === 0 ? <p className="text-sm text-white/40">Nothing hidden.</p> : (
        <div className="flex flex-col gap-2">
          {data.map((c) => (
            <div key={c.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-2.5">
              {c.thumbnail && /* eslint-disable-next-line @next/next/no-img-element */ <img src={c.thumbnail} alt="" className="h-10 w-16 shrink-0 rounded object-cover" />}
              <a href={`/content/${c.id}`} target="_blank" rel="noreferrer" className="min-w-0 flex-1 truncate text-sm text-white hover:underline">{c.title}</a>
              <button onClick={() => restore(c.id)} className="flex items-center gap-1 rounded-full bg-green-600/80 px-3 py-1.5 text-xs font-medium text-white hover:bg-green-600"><RotateCcw size={13} /> Restore</button>
            </div>
          ))}
        </div>
      )}
