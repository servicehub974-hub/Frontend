...
export function Header({
  ...
  const { data: site } = useQuery({ queryKey: ["site"], queryFn: () => apiGet<{ name: string; logo_url: string | null }>("/api/site") });
        ...
        <Link href="/" className="group flex items-center gap-2">
          <div className="relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg border border-white/10 bg-black">
            ...
            {site?.logo_url ? (
              ...
              <img src={site.logo_url} alt="" className="relative z-10 h-5 w-5 rounded object-cover" />
            ) : (
