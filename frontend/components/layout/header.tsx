...
import { Menu, Search, Bell, Gem, Crown, Zap, LogOut, Sparkles } from "lucide-react";
...
export function Header({ onToggleSidebar }: { onToggleSidebar: () => void }) {
  ...
  const { user, loading, logout, becomeCreator } = useAuth();
            ...
            <div className="relative">
              ...
              >
                ...
                <img
                  src={
                    user.avatar_url ??
                    `https://i.pravatar.cc/150?u=${user.username ?? user.id}`
                  }
                  ...
                  <div className="absolute right-0 top-12 z-50 w-56 glass-panel rounded-xl p-2">
                    ...
                    {user.role === "user" && (
                      <button
                        ...
                        className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-fuchsia transition-colors hover:bg-white/5"
                      >
