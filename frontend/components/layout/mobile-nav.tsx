...
import { Compass, Clapperboard, Rss, MessageSquare, User } from "lucide-react";
...
export function MobileBottomNav({ onProfile }: { onProfile: () => void }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 flex h-16 items-center justify-around border-t border-white/5 glass-panel px-2 pb-[env(safe-area-inset-bottom)] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] sm:hidden">
      ...
      <Link href="/feed" className="flex h-full w-full flex-col items-center justify-center">
        <Rss size={20} className={isActive("/feed") ? "text-white" : "text-white/40"} />
        <span className={`text-[9px] ${isActive("/feed") ? "text-white" : "text-white/40"}`}>Feed</span>
      </Link>
