...
export function ProfileMenu({
  ...
  const walletQ = useQuery({ queryKey: ["wallet"], queryFn: () => apiGet<{ gems: number }>("/api/wallet"), enabled: !!user });
  ...
  if (!user) {
    return (
      <>
        ...
        <div className="fixed z-[70] glass-panel bottom-0 left-0 right-0 rounded-t-2xl pb-[env(safe-area-inset-bottom)] sm:bottom-auto sm:left-auto sm:right-4 sm:top-[4.75rem] sm:w-80 sm:rounded-xl">
          <div className="mx-auto mt-2 h-1 w-10 rounded-full bg-white/15 sm:hidden" />
          <div className="p-5 text-center">
            <p className="mb-1 font-semibold text-white">You&apos;re browsing as a guest</p>
            <p className="mb-4 text-sm text-white/50">Log in to upload, message, and sync your activity.</p>
            <div className="flex flex-col gap-2">
              <Link href="/login" onClick={onClose} className="w-full rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-black hover:bg-white/90">Log in</Link>
              <Link href="/register" onClick={onClose} className="w-full rounded-full bg-white/10 px-4 py-2.5 text-sm font-semibold text-white hover:bg-white/15">Sign up</Link>
            </div>
          </div>
        </div>
      </>
  ...
  return (
    <>
      ...
      >
        ...
        <div className="px-2 py-1">
          <Row href="/gems" icon={Gem} label="Gems" onClose={onClose} right={<span className="text-xs font-semibold text-gold">{(walletQ.data?.gems ?? 0).toLocaleString()}</span>} />
