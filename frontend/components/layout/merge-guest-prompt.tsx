...
export function MergeGuestPrompt() {
  ...
  useEffect(() => {
    if (!user) return;
    if (localStorage.getItem("merge_seen")) return;
    apiGet<{ has: boolean }>("/api/users/me/anon-activity")
      .then((r) => { if (r.has) setShow(true); })
      .catch(() => {});
  }, [user]);

  if (!show) return null;

  const finish = () => { localStorage.setItem("merge_seen", "1"); setShow(false); };
  const merge = async () => {
    setBusy(true);
    try { await api("/api/users/me/merge-anon", { method: "POST" }); } catch { /* */ }
    finish();
  };
  const fresh = async () => {
    try { await api("/api/users/me/discard-anon", { method: "POST" }); } catch { /* */ }
    finish();
  };
