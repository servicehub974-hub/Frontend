...
import { Heart, MessageCircle, Repeat2, Share2, Bookmark, CheckCircle, Trash2, Send, Play, X, ChevronLeft, ChevronRight, Pencil, ImagePlus, Crown, Flag } from "lucide-react";
...
export function PostCard({ post, onDeleted, embedded }: { post: Post; onDeleted?: () => void; embedded?: boolean }) {
  ...
  const report = async () => { if (!user) { router.push("/login"); return; } const reason = prompt("Report this post — why?"); if (reason === null) return; try { await api("/api/report", { method: "POST", json: { target_type: "post", target_id: post.id, reason } }); alert("Thanks — reported to moderators."); } catch { /* */ } };
  ...
  return (
    <article className={embedded ? "rounded-xl border border-white/10 p-3" : "glass-panel rounded-2xl p-4"}>
      <header className="flex items-center gap-3">
        ...
        {!embedded && !post.is_owner && (
          <button onClick={report} className="rounded-full p-2 text-white/40 hover:bg-white/10 hover:text-white" title="Report"><Flag size={15} /></button>
        )}
      </header>
