...
function PostModal({ post, onClose }: { post: Post; onClose: () => void }) {
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-lg"
        onClick={onClose}
        onWheel={(e) => e.stopPropagation()}
        onTouchMove={(e) => e.stopPropagation()}
      />
      <div className="fixed inset-0 z-[201] flex items-start justify-center overflow-y-auto overscroll-contain p-4" onClick={onClose}>
        <div className="relative my-6 w-full max-w-xl rounded-2xl border border-white/10 bg-[#0d0d12] shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-white/10 bg-[#0d0d12] px-4 py-3">
          <span className="text-sm font-semibold text-white">{post.author.name}&apos;s post</span>
          <button onClick={onClose} className="rounded-full bg-white/10 p-1.5 text-white hover:bg-white/20"><X size={18} /></button>
        </div>
        <div className="p-4">
            <PostCard post={post} embedded />
            <div className="mt-4 border-t border-white/10 pt-4"><RichComments postId={post.id} /></div>
          </div>
        </div>
      </div>
    </>
