...
import { ThumbsUp, Pin, Trash2, CheckCircle, Crown, Flag } from "lucide-react";
...
function CommentItem({ c, contentId, ownerId, refresh }: {
  ...
  const report = async () => { if (!user) return; const reason = prompt("Report this comment — why?"); if (reason === null) return; try { await api("/api/report", { method: "POST", json: { target_type: "comment", target_id: c.id, reason } }); alert("Thanks — reported."); } catch { /* */ } };
    ...
    <div className="flex gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {c.author.vip_tier && <Crown size={12} className="text-fuchsia" />}
        ...
        <div className="mt-1.5 flex items-center gap-4 text-white/50">
          ...
          {!perms.del && <button onClick={report} className="text-xs hover:text-white" title="Report"><Flag size={12} /></button>}
        </div>
