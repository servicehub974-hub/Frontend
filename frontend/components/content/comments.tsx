...
import { ThumbsUp, Pin, Trash2, CheckCircle, MessageSquare } from "lucide-react";
...
function canModerate(user: User | null, c: CommentT, ownerId: string) {
  ...
  return {
    del: c.author.id === user.id || user.id === ownerId || admin,
...
function CommentBox({
  ...
  const submit = async () => {
    if (!body.trim()) return;
    setBusy(true);
    try { await onSubmit(body.trim()); setBody(""); } finally { setBusy(false); }
  };

  return (
    <div className="flex gap-3">
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={user.avatar_url ?? `https://i.pravatar.cc/150?u=${user.username ?? user.id}`}
        alt="" className={`${compact ? "h-7 w-7" : "h-9 w-9"} shrink-0 rounded-full border border-white/15 object-cover`} />
      <div className="flex-1">
        <textarea
          rows={compact ? 1 : 2} value={body} onChange={(e) => setBody(e.target.value)}
...
function CommentItem({
  ...
  const like = async () => {
    ...
    } catch { setLiked(c.is_liked); setLikeCount(c.like_count); }
  };
  ...
  const addReply = async (body: string) => {
    await api(`/api/content/${contentId}/comments`, { method: "POST", json: { body, parent_id: c.id } });
  ...
  const del = async () => {
    ...
    await api(`/api/comments/${c.id}`, { method: "DELETE" }); refresh();
  };
  const pin = async () => { await api(`/api/comments/${c.id}/pin`, { method: "POST" }); refresh(); };
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="min-w-0 flex-1">
        {c.is_pinned && (
          <div className="mb-0.5 flex items-center gap-1 text-[11px] text-white/40"><Pin size={11} /> Pinned</div>
        )}
        ...
        <div className="mt-1.5 flex items-center gap-4 text-white/50">
          <button onClick={like} className="flex items-center gap-1 text-xs hover:text-white">
            <ThumbsUp size={14} className={liked ? "fill-white text-white" : ""} /> {likeCount > 0 ? likeCount : ""}
          </button>
          {user && !c.parent_id && (
            <button onClick={() => setReplyOpen((v) => !v)} className="text-xs font-medium hover:text-white">Reply</button>
          )}
          {perms.pin && !c.parent_id && (
            <button onClick={pin} className="text-xs hover:text-white" title="Pin"><Pin size={13} /></button>
          )}
          {perms.del && (
            <button onClick={del} className="text-xs hover:text-rose" title="Delete"><Trash2 size={13} /></button>
          )}
        </div>

        {replyOpen && (
          <div className="mt-3">
            <CommentBox compact placeholder={`Reply to ${c.author.name}…`} onSubmit={addReply} />
          </div>
        )}
        ...
        {showReplies && replies && (
          <div className="mt-3 flex flex-col gap-4 border-l border-white/5 pl-3">
            {replies.map((r) => (
              <CommentItem key={r.id} c={r} contentId={contentId} ownerId={ownerId} refresh={async () => {
                setReplies(await apiGet<CommentT[]>(`/api/comments/${c.id}/replies`)); refresh();
              }} />
            ))}
          </div>
        )}
...
export function Comments({
  contentId, ownerId, count,
}: {
  contentId: string; ownerId: string; count: number;
}) {
  const { user } = useAuth();
  ...
  const addComment = async (body: string) => {
    await api(`/api/content/${contentId}/comments`, { method: "POST", json: { body } });
    refresh();
  };
  ...
  return (
    <div className="mt-6 px-4 sm:px-0">
      <div className="mb-4 flex items-center gap-4">
        <h2 className="text-base font-bold text-white">{count} Comments</h2>
      ...
      {user ? (
        ...
        <p className="mb-6 rounded-lg bg-white/5 px-4 py-3 text-sm text-white/50">
          <MessageSquare size={14} className="mr-2 inline" /> Log in to join the conversation.
        </p>
      )}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-5">
          {data.map((c) => (
            <CommentItem key={c.id} c={c} contentId={contentId} ownerId={ownerId} refresh={refresh} />
          ))}
        </div>
      )}
    </div>
