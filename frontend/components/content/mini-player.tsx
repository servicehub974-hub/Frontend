...
export function MiniPlayer() {
  ...
  useEffect(() => {
    ...
    if (el && mini) {
      try { el.currentTime = mini.time || 0; } catch { /* */ }
      el.play().catch(() => {});
    }
  }, [mini?.id, mini?.src]);

  if (!mini) return null;
  // hide while the full watch page for this video is open
  if (pathname === `/content/${mini.id}`) return null;

  const toggle = () => { const el = ref.current; if (!el) return; el.paused ? el.play() : el.pause(); };
  const maximize = () => router.push(`/content/${mini.id}`);
    ...
    <div className="fixed bottom-20 right-3 z-[120] w-72 max-w-[86vw] overflow-hidden rounded-xl border border-white/15 bg-black shadow-2xl sm:bottom-4 sm:right-4">
      <div className="relative">
        {mini.embed ? (
          ...
          <video ref={ref} src={mini.src} poster={mini.poster} className="aspect-video w-full bg-black" playsInline autoPlay onClick={toggle} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} />
        )}
        {!mini.embed && (
