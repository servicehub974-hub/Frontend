...
export function VideoPlayer({ src, poster, onNext }: { src: string; poster?: string; onNext?: () => void }) {
  ...
  const onKey = (e: React.KeyboardEvent) => {
    ...
    const handled = [" ", "k", "j", "l", "m", "f", "c", "arrowleft", "arrowright", "arrowup", "arrowdown", "home", "end", ",", ".", "<", ">"].includes(kl) || /^[0-9]$/.test(k);
    ...
    else if (kl === "arrowup") setVol(volume + 0.05);
    else if (kl === "arrowdown") setVol(volume - 0.05);
    else if (kl === "m") { if (el) { el.muted = !el.muted; setMuted(el.muted); } }
    else if (kl === "f") toggleFull();
    else if (kl === "home") seekTo(0);
    else if (kl === "end") { if (el) seekTo(el.duration); }
    else if (k === "<") changeSpeed(SPEEDS[Math.max(0, SPEEDS.indexOf(speed) - 1)]);
    else if (k === ">") changeSpeed(SPEEDS[Math.min(SPEEDS.length - 1, SPEEDS.indexOf(speed) + 1)]);
    else if (k === "," && el?.paused) seekTo(current - 1 / 30);
    else if (k === "." && el?.paused) seekTo(current + 1 / 30);
  ...
  return (
    <div ref={wrapRef} tabIndex={0} onKeyDown={onKey} onMouseMove={flashUI}
