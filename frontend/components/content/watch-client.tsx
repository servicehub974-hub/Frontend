...
import {
  ThumbsUp, ThumbsDown, Share2, Bookmark, Lock, ListPlus, Flag,
...
import { AddToPlaylist } from "@/components/content/add-to-playlist";
...
function Media({ d, contentId, startTime, onNext, related, onSelect }: { d: Detail; contentId: string; startTime?: number; onNext?: () => void; related?: any[]; onSelect?: (id: string) => void }) {
  ...
  return <VideoPlayer src={d.media_url} poster={d.thumbnail_url ?? undefined} onNext={onNext} related={related} onSelect={onSelect} contentId={contentId} title={d.title} startTime={startTime} />;
}
...
export function WatchClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    // view/history beacon (separate from the metadata fetch so prefetch is free)
    api(`/api/content/${id}/view`, { method: "POST" }).catch(() => {});
  }, [id]);
  const { user } = useAuth();
  const router = useRouter();
  const qc = useQueryClient();
  const messagesEnabled = useFeature("messages");
  const { closeMini, resume, clearResume } = useMiniPlayer();
  const [startAt] = useState(() => (resume && resume.id === id ? resume.time : 0));
  useEffect(() => { closeMini(); if (resume && resume.id === id) clearResume(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [id]);
  ...
  const onUnlock = async () => {
    ...
    setUnlockErr("");
    try {
      await api(`/api/content/${id}/unlock`, { method: "POST" });
      qc.invalidateQueries({ queryKey: ["content", id] });
      qc.invalidateQueries({ queryKey: ["wallet"] });
    } catch (e) { setUnlockErr((e as { message?: string })?.message || "Unlock failed"); }
  };

  const onReport = async () => {
    if (!user) return requireLogin();
    const reason = prompt("Report this video — why?");
    if (reason === null) return;
    try { await api("/api/report", { method: "POST", json: { target_type: "content", target_id: id, reason } }); alert("Thanks — reported to moderators."); } catch { /* */ }
  };
  const onSave = async () => {
    if (!user) return requireLogin();
        ...
        {locked ? (
          <div className="relative flex aspect-video w-full items-center justify-center overflow-hidden border border-fuchsia/20 bg-elevated sm:rounded-xl">
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              <Lock size={28} className="mb-3 text-fuchsia" />
              <p className="font-semibold text-white">Premium content</p>
              <p className="mt-1 text-sm text-white/50">Unlock this with Gems to watch.</p>
              <button onClick={onUnlock} className="mt-4 flex items-center gap-2 rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-black hover:bg-white/90">
                Unlock · {d.price_gems} 💎
              </button>
              {unlockErr && <p className="mt-2 text-xs text-rose">{unlockErr}</p>}
            </div>
          </div>
        ) : (
          <Media d={d} contentId={id} startTime={startAt} onNext={goNext} related={relatedItems} onSelect={goSelect} />
        )}
        ...
        <div className="px-4 sm:px-0">
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar">
              ...
              <ActionPill onClick={() => (user ? setShowPlaylist(true) : requireLogin())}><ListPlus size={16} /> Playlist</ActionPill>
              <ActionPill onClick={onReport}><Flag size={16} /> Report</ActionPill>
            </div>
          </div>
          ...
          <AddToPlaylist contentId={d.id} open={showPlaylist} onClose={() => setShowPlaylist(false)} />
        </div>
      </div>
