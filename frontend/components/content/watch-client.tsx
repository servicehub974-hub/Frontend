...
function Media({ d, contentId, onNext, related, onSelect }: { d: Detail; contentId: string; onNext?: () => void; related?: any[]; onSelect?: (id: string) => void }) {
  ...
  return <VideoPlayer src={d.media_url} poster={d.thumbnail_url ?? undefined} onNext={onNext} related={related} onSelect={onSelect} contentId={contentId} title={d.title} />;
}
...
export function WatchClient({ id }: { id: string }) {
  ...
  const { closeMini } = useMiniPlayer();
  useEffect(() => { closeMini(); }, [id, closeMini]);
    ...
    <div className="mx-auto max-w-[1800px] px-0 py-0 sm:px-4 sm:py-4 lg:flex lg:gap-6">
      <div className="min-w-0 lg:flex-1">
        {locked ? (
          ...
          <Media d={d} contentId={id} onNext={goNext} related={relatedItems} onSelect={goSelect} />
        )}
