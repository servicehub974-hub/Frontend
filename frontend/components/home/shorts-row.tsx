...
interface Reel { id: string; title: string; thumbnail_url: string | null; creator: { name: string }; like_count: number; }

export function ShortsRow() {
  const { data } = useQuery({ queryKey: ["shorts-row"], queryFn: () => apiGet<{ items: Reel[] }>("/api/reels") });
  ...
  return (
    <section className="my-6">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 hide-scrollbar">
        {items.map((r) => (
          <Link key={r.id} href="/reels" className="group relative aspect-[9/16] w-40 shrink-0 overflow-hidden rounded-xl bg-elevated sm:w-44">
            {r.thumbnail_url ? (
