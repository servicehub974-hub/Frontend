...
import { CheckCircle, Image as ImageIcon, Crown, Pencil, Trash2 } from "lucide-react";
...
export function ContentCard({ item, index, owner = false }: { item: ContentItem; index: number; owner?: boolean }) {
  const qc = useQueryClient();
  const delContent = async (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    if (!confirm("Delete this content?")) return;
    try { await api(`/api/content/${item.id}`, { method: "DELETE" }); qc.invalidateQueries(); } catch { /* */ }
  };
  const { user } = useAuth();
  const prefetch = () =>
    qc.prefetchQuery({
